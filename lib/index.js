...
module.exports = function(Model, filters) {
	...
	var mongooseService = {
		...
		get: function(id, params, callback) {
			...
			// If only two arguments are passed, we didn't receive an id.
			if (arguments.length < 3) {
				callback = arguments[1];
				params = arguments[0].query;
				query = Model.findOne(params); // findOne using the params.
			} else {
				query = Model.findById(id); // findById.
			}
